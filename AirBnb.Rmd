...
#### Federica Mori
I am Italian, born in Mantova in 1986. I've working within innovation for more than 10 years. Now I'm an Innovation Manager at Confindustria Emilia-Romagna Ricerca. 

#### Daniel Mota De Carvalho
Lorem ipsum dolor sit amet, consectetur adipisci elit, sed do eiusmod tempor incidunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrum exercitationem ullamco laboriosam, nisi ut aliquid ex ea commodi consequatur. Duis aute irure reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint obcaecat cupiditat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.

#### Virgínia Mendonça
I am Brazilian and have been living in EU the latest 10 years. I work with database management systems, most experienced in Sql Server and Postgres. Now adventuring myself in the data management world!
...
listings%>%
  select(price, host_response_time, host_is_superhost, calculated_host_listings_count, calculated_host_listings_count_entire_homes)%>%
  ggpairs(alpha = 0.3)
...
listings%>%
  select(price, number_of_reviews, review_scores_rating, review_scores_accuracy, review_scores_cleanliness, review_scores_checkin, review_scores_communication, review_scores_value)%>%
  ggpairs(alpha = 0.3)
...
*The total variability explained by this model is very low (only 1%) then 
...
model1a <- lm(price_4_nights ~ review_scores_rating, data = listings)
msummary(model1a)

```

*t-stat value is greater than the 2% and p-value is less than 5% then we can 
reject the hypothesis of no difference between variables. An extra review score
rating decrease the price by 352.7.*
*The total variability explained by this model is very low (only 0.5%) then 
scores of reviews is not a significant predictor for cost for 4 nights.*
...
model1b <- lm(price_4_nights ~ prop_type_simplified, data = listings)
msummary(model1b)

```

*For some type of property (i.e., Private room in condo and Private room in 
rental unit) t-stat value is greater than the 2% and p-value is less than 5% 
then we can reject the hypothesis of no difference between variables. For 
the other types of property (Entire rental unit and Other), t-stat value is
lower than the 2% then we can accept the hypothesis of no difference between 
variables.*
*In addition, an additional unit of some property type decrease the price (i.e., Entire rental of unit by 6.69, Other by 88.93, Private room in 
condo by 377.18, Private room in rental unit by 365.08) respect to Tiny room type*
*The total variability explained by this model is very low (only 0.4%) then 
...
*The total variability explained by the model2 is better (1.4%) then model1 but
remain very low so variables are not explanatory for cost for 4 nights.*

```{r regressionmodel2a_room_type}

model2a <- lm(price_4_nights ~ room_type, data = listings)
msummary(model2a)

```

*For some type of room (i.e., Hotel room, Private room) t-stat value is greater
than the 2% and p-value is less than 5% then we can reject the hypothesis of 
no difference between variables. For the other type of room (Shared room), 
t-stat value is lower than the 2% then we can accept the hypothesis of no 
difference between variables.*
*In addition, an additional unit of some type of room increase the price 
(i.e., Hotel room by 1022.9) or decrease the price (i.e. Private room of 
315.6 and Shared room of 423.4) respect to the type of Tiny room*
*The total variability explained by this model is very low (only 0.8%) then 
...
model3a <- lm(price_4_nights ~ n_bathrooms, data = listings)
msummary(model3a)

```

*t-stat value is greater than the 2% and p-value is less than 5% then we can 
reject the hypothesis of no difference between variables. An extra bathroom
increase the price by 185.9.*
*The total variability explained by this model is very low (only 0.2%) then 
numbers of bathrooms are not a significant predictor for cost for 4 nights.*

```{r regressionmodel3b_bedrooms}

model3b <- lm(price_4_nights ~ bedrooms, data = listings)
msummary(model3b)

```

*t-stat value is greater than the 2% and p-value is less than 5% then we can 
reject the hypothesis of no difference between variables. An extra bedroom
increase the price by 253.4.*
*The total variability explained by this model is very low (only 0.8%) then 
numbers of bathrooms are not a significant predictor for cost for 4 nights.*

```{r regressionmodel3c_beds}

model3c <- lm(price_4_nights ~ beds, data = listings)
msummary(model3c)

```

*t-stat value is greater than the 2% and p-value is less than 5% then we can 
reject the hypothesis of no difference between variables. An extra bed
increase the price by 92.5.*
*The total variability explained by this model is very low (only 0.4%) then 
numbers of bathrooms are not a significant predictor for cost for 4 nights.*

```{r regressionmodel3d_accommodates}

model3d <- lm(price_4_nights ~ accommodates, data = listings)
msummary(model3d)

```

*t-stat value is greater than the 2% and p-value is less than 5% then we can 
reject the hypothesis of no difference between variables. An extra accommodates
increase the price by 95.7.*
*The total variability explained by this model is very low (only 0.6%) then 
numbers of bathrooms are not a significant predictor for cost for 4 nights.*
...
model4 <- lm(price_4_nights ~ host_is_superhost, data = listings)
msummary(model4)
...
*In addition t-stat value is lower than the 2% and p-value is above than 5% 
then we can accept the hypothesis of no difference between variables. The 
condition of being a superhost decrease the price of the room by 112.2 respect 
if the host is not a superhost.*
*The total variability explained by this model is very low (only 0.04%) then 
numbers of bathrooms are not a significant predictor for cost for 4 nights.*
...
Determine whether location is a predictor of price_4_nights
...
model6 <- lm(price_4_nights ~ neighbourhood_simplified, data = listings)
msummary(model6)

```

*For some type of neighborhood (i.e., Private room in condo and Private room in 
rental unit) t-stat value is greater than the 2% and p-value is less than 5% 
then we can reject the hypothesis of no difference between variables. For 
the other types of property (Entire rental unit and Other), 
*t-stat value is lower than the 2% and p-value is above 5% then we can accept 
the hypothesis of no difference between variables.*
*In addition, an additional unit in some neighborhood increase the price
(i.e., Navile by 168, Porto - Saragozza by 190, San Donato - San Vitale by 164,
Santo Stefano by 179, Savena by 361) respect to Borgo Panigale - Reno.*
*The total variability explained by this model is very low (only 0.07%) then 
property types is not a significant predictor for cost for 4 nights.*

## to be deleted
*What is the effect of cancellation_policy on price_4_nights, after we control 
for other variables?*
